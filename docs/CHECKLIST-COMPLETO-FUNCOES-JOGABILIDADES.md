# Checklist técnico completo — OTTHI World Edu V642
...
- [x] Funções: **544**
- [x] Condições/loops capturados: **1533**
- [x] Bindings de eventos: **186**
- [x] Declarações de topo: **93**
- [x] Nós HTML: **263**
- [x] IDs HTML: **104**
...
- [x] `07-navigation-traffic-routes.js` — Rotas, trânsito, grafo, GPS e minimapa — 25 funções, 59 condições/loops, 0 eventos
...
- [x] `15-transit-bus-metro.js` — Ônibus, rotas viárias, paradas, metrô e painel de transporte — 33 funções, 107 condições/loops, 10 eventos
- [x] `16-emergency-services.js` — Bombeiros, incêndios, polícia, ambulância, segurança e incidentes — 26 funções, 73 condições/loops, 2 eventos
- [x] `17-adventures-learning-world.js` — Castelo, aventuras, desafios e praça educacional — 10 funções, 21 condições/loops, 1 eventos
- [x] `18-water-fishing-boats.js` — Água, câmera e animação da pesca, barco e física náutica — 32 funções, 60 condições/loops, 3 eventos
- [x] `19-campfire-hunting-house-extensions.js` — Fogueira, caça, animais e ampliações de casas — 24 funções, 33 condições/loops, 5 eventos
...
- [x] `23-vehicle-effects-driving.js` — Poeira, som, efeitos, controles, direção, passageiros e ponte — 11 funções, 43 condições/loops, 0 eventos
...
- [x] `26-input-player-physics.js` — Entrada, corrida, pulo, física do jogador/veículo e animação — 13 funções, 50 condições/loops, 0 eventos
- [x] `27-npc-enemies-combat-camera-action.js` — Sociedade NPC, inimigos, combate, câmera, contexto, ação e necessidades — 16 funções, 65 condições/loops, 0 eventos
...
- [x] `29-game-loop-controls-gamepad.js` — Loop principal, controles, gamepad e início do jogo — 3 funções, 33 condições/loops, 13 eventos
- [x] `30-pause-tests-public-api-bootstrap.js` — Pausa, testes de veículo, API pública de auditoria e bootstrap final — 5 funções, 22 condições/loops, 6 eventos
...
- [x] Declarações de função nomeadas: **544**
- [x] Funções arrow de topo: **17**
- [x] Tokens `=>` incluindo callbacks: **848**
- [x] Ocorrências de fluxo (`if/else/switch/case/for/while/catch/return/throw`): **2851**
- [x] Bindings de evento detalhados: **236**
- [x] Pares de dependência entre módulos: **271**

### Fluxo por tipo

- [x] `catch`: **34** ocorrências
- [x] `else`: **232** ocorrências
- [x] `for`: **283** ocorrências
- [x] `if`: **1417** ocorrências
- [x] `return`: **866** ocorrências
