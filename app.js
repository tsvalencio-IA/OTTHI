(() => {
  ...
  function openObjectivesPanel(){
    deriveMissionFlags();const job=state.career?.activeJob,progress=job?activeJobProgress(job):null;
  ...
  function currentMapLocations(){const all=[...MAP_LOCATIONS,...(window.OTTHI_ROOM_WORLD?.mapHouseLocations?.()||[]),...missionMapLocations()];const unique=new Map();for(const loc of all)if(loc?.id)unique.set(loc.id,loc);return[...unique.values()];}
  ...
  function openMap(){
    ...
    const markers=placements.map(({loc,left,top})=>{const active=loc.id===activeId?' active':'',selected=loc.id===mapSelectedId?' selected':'',mission=loc.missionTarget?' mission-target':'';return `<button class="map-marker clean${active}${selected}${mission}" style="left:${left.toFixed(2)}%;top:${top.toFixed(2)}%" data-map-marker="${loc.id}" aria-label="${loc.name}" title="${loc.name}"><b>${loc.icon}</b><span>${loc.name}</span></button>`;}).join('');
    ...
    openModal('Mapa',`<div class="map-layout v626"><div class="map-main"><div class="world-map clean-map"><i class="map-road horizontal"></i><i class="map-road vertical"></i><i class="map-road west"></i><i class="map-road east"></i><i class="map-river"></i><div class="map-region forest">FLORESTA</div><div class="map-region city">VILA</div><div class="map-region adventure">AVENTURA</div>${window.OTTHI_ROOM_WORLD?.mapRegionsMarkup?.(worldToMap)||''}${route.length?routeSvgMarkup(route):''}${markers}<span class="player-dot" style="left:${pp.left}%;top:${pp.top}%;--player-angle:${angleDeg}deg"><i></i><b>VOCÊ</b></span><span class="map-north">N</span></div>${current}<div id="mapSelection">${mapSelectionMarkup(mapSelectedId)}</div></div><aside class="map-sidebar"><h3>Escolha um lugar</h3><div class="map-destinations grouped">${grouped}</div></aside></div>`,root=>{
      const fitMapToViewport=()=>{const map=$('.clean-map',root),main=$('.map-main',root);if(!map||!main)return;const gps=$('.gps-current',main),selection=$('#mapSelection',main),style=getComputedStyle(main),gap=parseFloat(style.rowGap||style.gap||'0')||0,portrait=matchMedia?.('(orientation: portrait)')?.matches??innerHeight>=innerWidth,availableWidth=Math.max(96,main.clientWidth),reserved=(gps?.offsetHeight||0)+(selection?.offsetHeight||0)+gap*2,availableHeight=portrait?Math.max(180,Math.min(innerHeight*.43,420)):Math.max(96,main.clientHeight-reserved),size=Math.max(96,Math.floor(Math.min(availableWidth,availableHeight)));map.style.setProperty('width',`${size}px`,'important');map.style.setProperty('height',`${size}px`,'important');map.style.setProperty('min-height','0','important');map.style.setProperty('max-height',`${size}px`,'important');const rect=map.getBoundingClientRect();applyMapMarkerPlacements(root,mapMarkerPlacements(mapLocations,pp,rect.width,rect.height));};
      requestAnimationFrame(fitMapToViewport);setTimeout(fitMapToViewport,80);
      const selectMapLocation=id=>{mapSelectedId=id;$$('[data-map-marker]',root).forEach(x=>x.classList.toggle('selected',x.dataset.mapMarker===id));$$('[data-map-list]',root).forEach(x=>x.classList.toggle('selected',x.dataset.mapList===id));selection.innerHTML=mapSelectionMarkup(id);bindRoute();requestAnimationFrame(fitMapToViewport);if(matchMedia?.('(orientation: portrait)')?.matches)selection.scrollIntoView?.({behavior:'smooth',block:'nearest'});};
  ...
  function serviceVehicleAvailable(vehicle,job=null){if(!vehicle||vehicle.occupied||vehicle.incidentTargetId||vehicle.targetFireId||vehicle.group?.visible===false)return false;const token=job?.instanceId||state.career?.activeJob?.instanceId||'';return!vehicle.missionReservedFor||vehicle.missionReservedFor===token;}
  ...
  function registerServiceVehicle(vehicle,kind,index=0){
    ...
    registerInteractable({id:`service-vehicle-${vehicle.id}`,type:'serviceVehicle',icon:kind==='police'?'🚓':kind==='firefighter'?'🚒':'🚑',label:`Entrar: ${vehicle.label}`,radius:kind==='firefighter'?3.5:3.0,priority:238,getPos:()=>({x:vehicle.group.position.x,z:vehicle.group.position.z}),available:()=>serviceVehicleAvailable(vehicle,state.career?.activeJob),action:()=>enterServiceVehicle(vehicle)});return vehicle;
  }
  function enterServiceVehicle(vehicle){
    const job=state.career?.activeJob,required=serviceKindForJob(job),kind=String(vehicle?.serviceType||'');if(required&&required!==kind){toast(`Esta missão exige ${required==='police'?'uma viatura':required==='firefighter'?'um caminhão dos bombeiros':'uma ambulância'}.`,'warn',2400);return false;}if(job?.serviceVehicleId&&job.serviceVehicleId!==vehicle?.id){const assigned=serviceMissionVehicle(job);toast(`Use ${assigned?.label||'o veículo indicado'} marcado no GPS para esta missão.`,'warn',2500);return false;}if(!serviceVehicleAvailable(vehicle,job)){toast('Este veículo está atendendo outra ocorrência.','warn');return false;}return enterVehicle(vehicle);
  }
  function serviceMissionVehicle(job=state.career?.activeJob){return job?.serviceVehicleId?serviceVehicleById(job.serviceVehicleId):null;}
  function serviceVehicleMapLocation(job=state.career?.activeJob){const vehicle=serviceMissionVehicle(job),kind=serviceKindForJob(job);if(!vehicle||job?.serviceVehicleBoarded)return null;const icon=kind==='police'?'🚓':kind==='firefighter'?'🚒':'🚑',x=Number(vehicle.group?.position?.x||vehicle.home?.x||0),z=Number(vehicle.group?.position?.z||vehicle.home?.z||0);return{id:`service-${vehicle.id}`,name:`${vehicle.label} da missão`,icon,group:'Missão ativa',x,z,navX:x,navZ:z,missionTarget:true,description:`Este é o ${vehicle.label.toLowerCase()} reservado exclusivamente para a missão atual.`,actions:['Vista o uniforme','Siga a rota azul','Chegue perto e use AÇÃO para entrar']};}
  function reserveMissionServiceVehicle(job=state.career?.activeJob,options={}){
  ...
  function onServiceVehicleBoarded(vehicle){const job=state.career?.activeJob,kind=String(vehicle?.serviceType||'');if(!job||serviceKindForJob(job)!==kind)return;job.serviceVehicleId=vehicle.id;job.serviceVehicleBoarded=true;job.serviceVehicleArrived=false;setMissionState(job,MISSION_STATES.TRAVELLING,'service-vehicle-boarded');focusActiveJob();saveState(true);}
  function onServiceVehicleExited(vehicle){const job=state.career?.activeJob;if(!job||job.serviceVehicleId!==vehicle?.id)return;job.lastVehicleExitAt=Date.now();saveState();}
