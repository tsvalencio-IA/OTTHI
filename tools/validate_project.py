...
def main():
    ...
    add('31 módulos JavaScript',len(js)==31,len(js)); add('12 módulos CSS',len(css)==12,len(css)); add('Manifesto JS completo',len(manifest.get('javascript',[]))==31); add('Manifesto CSS completo',len(manifest.get('styles',[]))==12)
    ...
    add('IDs HTML únicos',not dup,dup);add('Referências locais existem',not missing,missing);add('Versão 643 no index',index.count('?v=643')>=10,index.count('?v=643'));add('GitHub Pages OTTHI',('OTTHI'+'-WORLD-EDU') not in index)
    ...
    sw=(ROOT/'sw.js').read_text('utf-8');add('Service Worker V643',"otthi-v643-1" in sw and "643.0-precision-mobility-traffic-fishing" in sw)
