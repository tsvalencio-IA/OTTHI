...
function validRoomSlots(value={},capacity=10){const allowed=new Set(fixedRoomSlotKeys(capacity));return Object.entries(value||{}).filter(([key,item])=>allowed.has(key)&&item&&typeof item==='object'&&item.uid);}
...
async function reserveRoomSlot(roomId,name=ownName()){
  ...
  const next=sanitizeRoom(roomId),capacity=Math.min(10,roomCapacity(next)),slotsRef=f.ref(db,`${ROOT}/rooms/${next}/slots`),snapshot=await f.get(slotsRef),slots=snapshot.val()||{},keys=fixedRoomSlotKeys(capacity),existingEntry=validRoomSlots(slots,capacity).find(([,item])=>item.uid===user.uid),joinedAt=Number(existingEntry?.[1]?.joinedAtClient||Date.now()),playerName=String(name||'Jogador').slice(0,24);
  for(const [legacyKey,item] of Object.entries(slots)){if(item?.uid===user.uid&&!keys.includes(legacyKey))await f.remove(f.ref(db,`${ROOT}/rooms/${next}/slots/${legacyKey}`)).catch(()=>{});}
  const candidates=existingEntry?[existingEntry[0],...keys.filter(key=>key!==existingEntry[0])]:keys;
  for(const slotKey of candidates){
    const slotRef=f.ref(db,`${ROOT}/rooms/${next}/slots/${slotKey}`),payload={slot:slotKey,uid:user.uid,name:playerName,room:next,joinedAtClient:joinedAt,updatedAtClient:Date.now()};
    try{
      const tx=await f.runTransaction(slotRef,current=>{if(current&&current.uid!==user.uid)return;return payload},{applyLocally:false});
      const value=tx.snapshot.val();
      if(tx.committed&&value?.uid===user.uid){await f.onDisconnect(slotRef).remove();const latest=await f.get(slotsRef),count=roomSlotCount(latest.val()||{},capacity);roomCountsCache[next]=count;dispatchRoomCounts();return{ok:true,room:next,count,capacity,slotRef,slotKey};}
    }catch(error){if(String(error?.code||'').includes('PERMISSION_DENIED'))throw error;}
  }
