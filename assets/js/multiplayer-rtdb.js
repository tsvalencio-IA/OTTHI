...
function normalizeParentalControls(value={}){const communicationEnabled=value?.communicationEnabled===true&&value?.chatEnabled===true,sessionLimitMinutes=[0,15,30,45,60].includes(Number(value?.sessionLimitMinutes))?Number(value.sessionLimitMinutes):0;return{multiplayerEnabled:value?.multiplayerEnabled!==false,communicationEnabled,chatEnabled:communicationEnabled,sessionLimitMinutes}}
...
function rememberGuestParentalControls(value={}){const clean=normalizeParentalControls({...value,communicationEnabled:false,chatEnabled:false});try{localStorage.setItem(GUEST_GUARDIAN_KEY,JSON.stringify(clean))}catch{}return clean}
function multiplayerAllowed(){return parentalControls.multiplayerEnabled!==false}
function chatAllowed(){return multiplayerAllowed()&&parentalControls.communicationEnabled!==false&&parentalControls.chatEnabled!==false}
...
async function reserveRoomSlot(roomId,name=ownName()){
  ...
  }catch(error){if(String(error?.code||error?.message||'').match(/permission|denied/i))throw error}
...
async function connect(options={}){
  ...
  connecting=(async()=>{const cfg=getConfig();if(!validConfig(cfg)){dispatch('otthos:mp-status',status({mode:'offline',error:'Firebase não configurado'}));return false}
    ...
    try{
      ...
      unsubs.push(f.onValue(f.ref(db,'.info/connected'),async s=>{connected=s.val()===true&&multiplayerAllowed();if(connected){await f.onDisconnect(refs.presence).remove().catch(()=>{});await f.onDisconnect(refs.slot).remove().catch(()=>{});await f.update(refs.profile,{name:ownName(),updatedAt:f.serverTimestamp()});await publish(lastPresence,true)}dispatch('otthos:mp-status',status({mode:connected?'firebase-public':'offline',disabled:!multiplayerAllowed(),error:connected?'':multiplayerAllowed()?'Sem conexão':'O modo online foi desativado pelos responsáveis.'}))},listenerError('conexão')));
      ...
      const emitSocialRequest=s=>{const value=s.val()||{};if(isBlocked(value.fromUid))return;requestCache[s.key]=value;dispatch('otthos:social-request',{id:s.key,...value})};
...
async function publish(payload,force=false){
  ...
  try{
    ...
    const lost=/reserva|permission|denied/i.test(String(error?.message||error||''));if(lost){connected=false;await api.remove(refs.presence).catch(()=>{});dispatch('otthos:mp-status',status({mode:'offline',error:'A vaga deste bairro expirou. Entre novamente.',full:true}));setTimeout(()=>disconnect().catch(()=>{}),0);}else listenerError('publicação')(error);return false;
  }
}
...
async function blockPlayer(targetUid){
  ...
  try{const record={uid:targetUid,name:sanitizePublicName(presenceCache[targetUid]?.name||'Jogador'),createdAt:api.serverTimestamp()};await api.set(api.ref(db,`${ROOT}/users/${user.uid}/blocks/${targetUid}`),record);blockedCache[targetUid]=record;delete presenceCache[targetUid];dispatch('otthos:mp-leave',{uid:targetUid,blocked:true});dispatch('otthi:blocked-players',{players:{...blockedCache}});return{ok:true}}catch(error){return{ok:false,error:error?.message||String(error)}}
}
async function unblockPlayer(targetUid){
  ...
  try{await api.remove(api.ref(db,`${ROOT}/users/${user.uid}/blocks/${targetUid}`));delete blockedCache[targetUid];dispatch('otthi:blocked-players',{players:{...blockedCache}});return{ok:true}}catch(error){return{ok:false,error:error?.message||String(error)}}
}
...
async function reportPlayer(targetUid,reason='other'){
  ...
  try{const reportRef=api.push(api.ref(db,`${ROOT}/reports`));await api.set(reportRef,{reporterUid:user.uid,targetUid,reason,room:currentRoom(),createdAt:api.serverTimestamp()});return{ok:true,id:reportRef.key}}catch(error){return{ok:false,error:error?.message||String(error)}}
}
async function sendChat(text){text=String(text||'').trim();if(!connected||!chatAllowed()||!APPROVED_CHAT_PHRASES.includes(text))return false;try{const r=api.push(refs.chat);await api.set(r,{senderUid:user.uid,name:ownName(),text,createdAt:api.serverTimestamp()});return true}catch(error){listenerError('chat')(error);return false}}
...
async function sendGift(targetUid,gift){
  if(!connected||!chatAllowed()||!targetUid||targetUid===user.uid||isBlocked(targetUid))return false;const type=gift?.type,amount=type==='coins'?10:type==='crystal'?1:0;if(!amount)return false;
  const r=api.push(api.ref(db,`${ROOT}/users/${targetUid}/inbox`));await api.set(r,{senderUid:user.uid,senderName:ownName(),type,amount,createdAt:api.serverTimestamp()});return true;
}
async function sendInteraction(targetUid,event={}){
  if(!connected||!chatAllowed()||!targetUid||targetUid===user.uid||isBlocked(targetUid))return false;const type=String(event.type||'');if(!INTERACTION_TYPES.includes(type))return false;
  ...
  const r=api.push(api.ref(db,`${ROOT}/users/${targetUid}/interactions`));await api.set(r,record);return true;
}
...
async function sendSocialRequest(targetUid,actionType,targetName='Jogador',extra={}){
  if(!connected||!chatAllowed()||!targetUid||targetUid===user.uid||isBlocked(targetUid))return{ok:false,error:'Jogador indisponível ou comunicação desativada'};
...
async function respondSocialRequest(requestId,decision){
  if(!connected||!chatAllowed()||!requestId)return{ok:false,error:'Solicitação inválida ou comunicação desativada'};const ref=api.ref(db,`${ROOT}/users/${user.uid}/socialRequests/${requestId}`);
  try{
    ...
    await api.update(ref,{status,respondedAt:api.serverTimestamp(),respondedAtClient:now});requestCache[requestId]={...request,status,respondedAtClient:now};await sendInteraction(request.fromUid,{type:'socialRequestResult',requestId,actionType:request.actionType,status});
...
async function completeSocialRequest(requestId){if(!connected||!chatAllowed()||!requestId)return false;const ref=api.ref(db,`${ROOT}/users/${user.uid}/socialRequests/${requestId}`);try{const snap=await api.get(ref),v=snap.val();if(!v||v.toUid!==user.uid)return false;await api.update(ref,{status:'completed',completedAt:api.serverTimestamp(),respondedAtClient:Date.now()});setTimeout(()=>api.remove(ref).catch(()=>{}),1200);return true}catch{return false}}
...
async function respondGameChallenge(challengeId,accepted){
  if(!connected||!chatAllowed()||!challengeId)return{ok:false,error:'Convite indisponível ou comunicação desativada'};const chRef=api.ref(db,`${ROOT}/users/${user.uid}/challenges/${challengeId}`),snap=await api.get(chRef),challenge=snap.val();if(!challenge)return{ok:false,error:'Convite não encontrado'};if(isBlocked(challenge.fromUid))return{ok:false,error:'Jogador bloqueado'};
...
async function finalizeGameSession(sessionId){
  if(!connected||!chatAllowed()||!sessionId)return{ok:false};const ref=api.ref(db,`${ROOT}/${WORLD}/gameSessions/${sessionId}`);
  try{
...
async function createCoopMission(type,options={}){
  if(!connected||!chatAllowed()||!refs.coopMissions||!user)return{ok:false,error:'Entre no modo online e libere comunicação para criar uma equipe.'};
  ...
  const record={id,type,title:String(options.title||type).slice(0,60),icon:String(options.icon||'🤝').slice(0,8),hostUid:user.uid,room:currentRoom(),status:'lobby',mode:options.mode==='solo'?'solo':'coop',minPlayers:Math.max(1,Math.min(maxPlayers,Number(options.minPlayers||1))),maxPlayers,createdAt:api.serverTimestamp(),createdAtClient:now,updatedAt:api.serverTimestamp(),updatedAtClient:now,participants:{[user.uid]:{uid:user.uid,name:ownName(),role,ready:false,joinedAt:api.serverTimestamp(),joinedAtClient:now,updatedAtClient:now}},progress:{phase:0,counter:0,target:Number(options.target||1),events:{},updatedAt:api.serverTimestamp(),updatedAtClient:now}};
  try{await api.set(missionRef,record);const participantRef=api.ref(db,`${ROOT}/${WORLD}/coopMissions/${id}/participants/${user.uid}`);await api.onDisconnect(participantRef).remove().catch(()=>{});coopMissionCache[id]=coopPublicRecord(record);dispatch('otthi:coop-missions',{missions:{...coopMissionCache}});return{ok:true,id,mission:coopMissionCache[id]}}catch(error){listenerError('criação de missão cooperativa')(error);return{ok:false,error:error?.message||String(error)}}
}
async function joinCoopMission(missionId,role='helper'){
  if(!connected||!chatAllowed()||!user||!missionId)return{ok:false,error:'Missão indisponível.'};role=COOP_ROLES.includes(role)?role:'helper';const ref=api.ref(db,`${ROOT}/${WORLD}/coopMissions/${missionId}`),now=Date.now();
  try{const tx=await api.runTransaction(ref,current=>{if(!current||current.status!=='lobby'||current.room!==currentRoom())return;const participants=current.participants&&typeof current.participants==='object'?{...current.participants}:{};const existing=participants[user.uid],count=Object.keys(participants).filter(uid=>participants[uid]).length;if(!existing&&count>=Math.max(1,Math.min(3,Number(current.maxPlayers||3))))return;participants[user.uid]={uid:user.uid,name:ownName(),role,ready:existing?.ready===true,joinedAt:existing?.joinedAt||api.serverTimestamp(),joinedAtClient:Number(existing?.joinedAtClient||now),updatedAtClient:now};return{...current,participants,updatedAt:api.serverTimestamp(),updatedAtClient:now}},{applyLocally:false});if(!tx.committed)return{ok:false,error:'A equipe já começou ou está completa.'};const participantRef=api.ref(db,`${ROOT}/${WORLD}/coopMissions/${missionId}/participants/${user.uid}`);await api.onDisconnect(participantRef).remove().catch(()=>{});const mission={id:missionId,...tx.snapshot.val()};coopMissionCache[missionId]=coopPublicRecord(mission);return{ok:true,id:missionId,mission:coopMissionCache[missionId]}}catch(error){return{ok:false,error:error?.message||String(error)}}
}
...
async function setCoopMissionStatus(missionId,status,options={}){if(!connected||!user||!missionId)return{ok:false,error:'Equipe indisponível.'};if(!['lobby','active','completed','cancelled'].includes(status))return{ok:false,error:'Estado inválido.'};try{const mission=coopMissionCache[missionId];if(!mission||mission.hostUid!==user.uid)return{ok:false,error:'Somente quem criou a equipe pode iniciar ou encerrar.'};const mode=options.mode==='solo'?'solo':options.mode==='coop'?'coop':mission.mode;await api.update(api.ref(db,`${ROOT}/${WORLD}/coopMissions/${missionId}`),{status,mode,startedAt:status==='active'?api.serverTimestamp():mission.startedAt||null,completedAt:status==='completed'?api.serverTimestamp():mission.completedAt||null,updatedAt:api.serverTimestamp(),updatedAtClient:Date.now()});return{ok:true,mode}}catch(error){return{ok:false,error:error?.message||String(error)}}}
async function updateCoopMissionProgress(missionId,patch={}){if(!connected||!user||!missionId)return{ok:false,error:'Missão cooperativa indisponível.'};const ref=api.ref(db,`${ROOT}/${WORLD}/coopMissions/${missionId}/progress`),now=Date.now();try{const tx=await api.runTransaction(ref,current=>{const mission=coopMissionCache[missionId];if(!mission?.participants?.[user.uid]||mission.status!=='active')return;const next={...(current||{}),updatedAt:api.serverTimestamp(),updatedAtClient:now};if(Number.isFinite(Number(patch.phase)))next.phase=Math.max(Number(current?.phase||0),Math.min(12,Number(patch.phase)));if(Number.isFinite(Number(patch.counterSet)))next.counter=Math.max(0,Math.min(999,Number(patch.counterSet)));else if(Number.isFinite(Number(patch.counterDelta)))next.counter=Math.max(0,Number(current?.counter||0)+Math.max(0,Math.min(20,Number(patch.counterDelta))));if(Number.isFinite(Number(patch.target)))next.target=Math.max(1,Math.min(99,Number(patch.target)));if(patch.eventId){const id=String(patch.eventId).replace(/[^A-Za-z0-9_-]/g,'').slice(0,80);if(id){next.events={...(current?.events||{})};next.events[id]={uid:user.uid,role:String(mission.participants[user.uid]?.role||'helper'),kind:String(patch.kind||'action').slice(0,24),value:Math.max(0,Math.min(99,Number(patch.value||1))),createdAt:api.serverTimestamp(),createdAtClient:now}}}return next},{applyLocally:false});return{ok:!!tx.committed,progress:tx.snapshot?.val()||null}}catch(error){return{ok:false,error:error?.message||String(error)}}}
async function leaveCoopMission(missionId){if(!connected||!user||!missionId)return false;try{await api.remove(api.ref(db,`${ROOT}/${WORLD}/coopMissions/${missionId}/participants/${user.uid}`));return true}catch{return false}}
