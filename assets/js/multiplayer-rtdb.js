...
const sanitizeRoom=value=>String(value||'bairro-central').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9-]/g,'-').replace(/-+/g,'-').replace(/^-|-$/g,'').slice(0,32)||'bairro-central';
let ROOM_ID=sanitizeRoom(window.OTTHI_CONFIG?.defaultRoom||'bairro-central'),WORLD=`rooms/${ROOM_ID}`;
...
function configuredRooms(){return Array.isArray(window.OTTHI_CONFIG?.rooms)?window.OTTHI_CONFIG.rooms:[]}
function roomCapacity(room=currentRoom()){return Number(configuredRooms().find(item=>item.id===room)?.capacity||window.OTTHI_CONFIG?.multiplayer?.maxPlayersPerRoom||10)}
function status(extra={}){return{configured:validConfig(getConfig()),connected,uid:user?.uid||'',room:currentRoom(),count:Object.keys(presenceCache).length,capacity:roomCapacity(),roomCounts:{...roomCountsCache},players:Object.entries(presenceCache).map(([uid,data])=>({uid,...data})),...extra}}
...
async function watchRoomCounts(){for(const off of roomCountUnsubs.splice(0)){try{off()}catch{}}if(!api||!db)return;for(const room of configuredRooms()){const roomId=sanitizeRoom(room.id),slotsRef=api.ref(db,`${ROOT}/rooms/${roomId}/slots`);roomCountUnsubs.push(api.onValue(slotsRef,snap=>{roomCountsCache[roomId]=snap.size||Object.keys(snap.val()||{}).length;dispatchRoomCounts()},listenerError(`vagas ${roomId}`)));}}
async function refreshRoomCounts(){if(!api||!db){try{await ensureServices()}catch{return{...roomCountsCache}}}await Promise.all(configuredRooms().map(async room=>{try{const snap=await api.get(api.ref(db,`${ROOT}/rooms/${sanitizeRoom(room.id)}/slots`));roomCountsCache[room.id]=snap.size||Object.keys(snap.val()||{}).length}catch{}}));dispatchRoomCounts();return{...roomCountsCache}}
async function reserveRoomSlot(roomId,name=ownName()){const f=await ensureServices();user=auth.currentUser||(await f.signInAnonymously(auth)).user;window.OTTHOS_RTDB.uid=user.uid;const next=sanitizeRoom(roomId),capacity=roomCapacity(next),slotsRef=f.ref(db,`${ROOT}/rooms/${next}/slots`),slotRef=f.ref(db,`${ROOT}/rooms/${next}/slots/${user.uid}`),snapshot=await f.get(slotsRef),slots=snapshot.val()||{},existing=!!slots[user.uid],count=Object.keys(slots).length;if(!existing&&count>=capacity)return{ok:false,full:true,room:next,count,capacity};try{await f.onDisconnect(slotRef).remove();await f.set(slotRef,{uid:user.uid,name:String(name||'Jogador').slice(0,24),room:next,joinedAtClient:Number(slots[user.uid]?.joinedAtClient||Date.now()),updatedAtClient:Date.now()});roomCountsCache[next]=existing?count:count+1;dispatchRoomCounts();return{ok:true,room:next,count:roomCountsCache[next],capacity,slotRef}}catch(error){const latest=await f.get(slotsRef).catch(()=>null),latestCount=latest?(latest.size||Object.keys(latest.val()||{}).length):count;return{ok:false,full:latestCount>=capacity,room:next,count:latestCount,capacity,error:latestCount>=capacity?'Bairro lotado. Escolha outro bairro.':(error?.message||String(error))}}}
...
async function connect(options={}){
  const requestedRoom=sanitizeRoom(options.room||getConfig()?.room||currentRoom());
  ...
  connecting=(async()=>{const cfg=getConfig();if(!validConfig(cfg)){dispatch('otthos:mp-status',status({mode:'offline',error:'Firebase não configurado'}));return false}
    try{
      ...
      const reservation=options.slotReservation?.ok&&options.slotReservation.room===ROOM_ID?options.slotReservation:await reserveRoomSlot(ROOM_ID,String(options.name||'Jogador').slice(0,24));
      if(!reservation.ok){dispatch('otthos:mp-status',status({mode:'offline',error:reservation.full?'Bairro lotado':'Não foi possível reservar uma vaga',full:!!reservation.full,count:reservation.count,capacity:reservation.capacity}));return false;}
      refs.slot=reservation.slotRef;refs.presence=f.ref(db,`${ROOT}/${WORLD}/presence/${user.uid}`);refs.presences=f.ref(db,`${ROOT}/${WORLD}/presence`);refs.profile=f.ref(db,`${ROOT}/users/${user.uid}/profile`);refs.progress=f.ref(db,`${ROOT}/users/${user.uid}/progress`);refs.learning=f.ref(db,`${ROOT}/users/${user.uid}/learning`);refs.houses=f.ref(db,`${ROOT}/${WORLD}/houses`);refs.campfires=f.ref(db,`${ROOT}/${WORLD}/campfires`);refs.extensions=f.ref(db,`${ROOT}/${WORLD}/houseExtensions`);refs.boats=f.ref(db,`${ROOT}/${WORLD}/boats`);refs.chat=f.ref(db,`${ROOT}/${WORLD}/chat`);refs.gifts=f.ref(db,`${ROOT}/users/${user.uid}/inbox`);refs.interactions=f.ref(db,`${ROOT}/users/${user.uid}/interactions`);refs.challenges=f.ref(db,`${ROOT}/users/${user.uid}/challenges`);refs.socialRequests=f.ref(db,`${ROOT}/users/${user.uid}/socialRequests`);refs.sessions=f.ref(db,`${ROOT}/${WORLD}/gameSessions`);lastPresence={name:String(options.name||'Jogador').slice(0,24),room:currentRoom(),x:0,y:0,z:0,r:0,color:0x5ad8ff};
      await watchRoomCounts();
      unsubs.push(f.onValue(f.ref(db,'.info/connected'),async s=>{connected=s.val()===true;if(connected){await f.onDisconnect(refs.presence).remove().catch(()=>{});await f.onDisconnect(refs.slot).remove().catch(()=>{});await f.update(refs.profile,{name:ownName(),updatedAt:f.serverTimestamp()});await publish(lastPresence,true)}dispatch('otthos:mp-status',status({mode:connected?'firebase-public':'offline',error:connected?'':'Sem conexão'}))},listenerError('conexão')));
    ...
    }catch(error){connected=false;dispatch('otthos:mp-status',status({mode:'offline',error:error?.message||String(error)}));console.warn('Firebase multiplayer:',error);return false}finally{connecting=null}
  })();return connecting;
}
async function publish(payload,force=false){lastPresence={...lastPresence,...payload,name:String(payload.name||lastPresence?.name||'Jogador').slice(0,24),emoteType:String(payload.emoteType||lastPresence?.emoteType||'').slice(0,16),emoteSeq:Number(payload.emoteSeq??lastPresence?.emoteSeq??0)};if(!connected||!refs.presence||!api)return false;const nowPerf=performance.now();if(!force&&nowPerf-presenceWrite<200)return false;presenceWrite=nowPerf;try{await api.update(refs.presence,{...lastPresence,updatedAt:api.serverTimestamp()});if(refs.slot&&(force||nowPerf-slotTouchAt>5000)){slotTouchAt=nowPerf;await api.update(refs.slot,{name:ownName(),room:currentRoom(),updatedAtClient:Date.now()}).catch(()=>{});}if(activeBoatLock&&lastPresence.boating&&lastPresence.boatRole==='driver'&&(force||nowPerf-boatTouchAt>5000)){boatTouchAt=nowPerf;await api.update(api.ref(db,`${ROOT}/${WORLD}/boats/${activeBoatLock}`),{driverName:ownName(),room:currentRoom(),updatedAt:api.serverTimestamp(),updatedAtClient:Date.now()}).catch(()=>{});}return true}catch(error){listenerError('publicação')(error);return false}}
...
async function sendSocialRequest(targetUid,actionType,targetName='Jogador',extra={}){
  ...
  try{
    ...
    const request={fromUid:user.uid,fromName:ownName(),toUid:targetUid,toName:String(targetName||'Jogador').slice(0,24),actionType,room:String(lastPresence?.room||currentRoom()).slice(0,32),status:'pending',createdAt:api.serverTimestamp(),createdAtClient:now,expiresAt:now+30000,extra:safeExtra};
...
async function setRoom(room){
  ...
  try{
    ...
    if(reservation?.slotRef&&api)await api.remove(reservation.slotRef).catch(()=>{});ROOM_ID=previous;WORLD=`rooms/${ROOM_ID}`;await connect({name,room:ROOM_ID}).catch(()=>false);dispatch('otthi:room-changed',{room:ROOM_ID,previousRoom:next,connected});return{ok:false,room:ROOM_ID,error:error?.message||String(error)};
  }
}
